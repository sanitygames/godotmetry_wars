...
func update_physics(delta: float) -> void:
	for x in range(grid_size.x):
		for y in range(grid_size.y):
			...
			# 1. 元の位置に戻ろうとする復元力
			var force = (pt.rest_position - pt.position) * spring_k

			# 2. 隣接する質点との相互作用（波の拡散）
			if x > 0:
				var left = points[x - 1][y] as Point
				force += (left.position - pt.position) * neighbor_k * 0.1
			if x < grid_size.x - 1:
				var right = points[x + 1][y] as Point
				force += (right.position - pt.position) * neighbor_k * 0.1
			if y > 0:
				var up = points[x][y - 1] as Point
				force += (up.position - pt.position) * neighbor_k * 0.1
			if y < grid_size.y - 1:
				var down = points[x][y + 1] as Point
				force += (down.position - pt.position) * neighbor_k * 0.1

			# 3. 速度と位置の適用 + 減衰
			pt.velocity += force * delta
			pt.velocity *= damping
